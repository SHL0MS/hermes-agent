...
**Use monospace fonts for all text.** Manim's Pango renderer produces broken kerning with proportional fonts (Helvetica, Inter, Arial) at all sizes. Monospace fonts have fixed character widths — zero kerning issues. See `references/visual-design.md` for full recommendations.

...
**Use monospace fonts for all text.** Manim's Pango text renderer produces broken kerning with proportional fonts (Helvetica, Inter, SF Pro, Arial) at all sizes and resolutions. Characters overlap, spacing is inconsistent, and the result looks unprofessional. This is a fundamental Pango limitation, not a Manim bug.
...
```python
# OK: large title, short string
Text("Manim", font_size=64, font="Helvetica Neue", weight=BOLD)

# BAD: small proportional text — kerning will break
Text("Approximation converges", font_size=18, font="Helvetica Neue")

# GOOD: small monospace text — consistent spacing
Text("Approximation converges", font_size=18, font="Menlo")
```
...
`"Menlo"` is the safest default — pre-installed on macOS, and Linux systems fall back to DejaVu Sans Mono which has identical metrics.

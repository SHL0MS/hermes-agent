...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay = float(os.environ.get("HERMES_DISCORD_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay = float(os.environ.get("HERMES_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
    ...
    def _text_batch_key(self, event) -> str:
        """Key for grouping rapid messages from the same user in the same channel."""
        return f"{event.source.chat_id}:{event.source.user_id}"

    def _enqueue_text_event(self, event) -> None:
    ...
    async def _flush_text_batch(self, key: str) -> None:
        ...
        try:
            ...
            if last_len >= self._SPLIT_THRESHOLD:
                delay = self._text_batch_split_delay
            else:
                delay = self._text_batch_delay
